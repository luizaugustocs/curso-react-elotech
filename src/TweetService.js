...
export const newTweet = async (tweetContent) => {
  ...
  return firebase.firestore().collection(`/users/${currentUser.uid}/followers`).get()
    .then(snapshot =>
      Promise.all(
        snapshot.docs
          .map(follower => firebase.firestore()
            .doc(`/users/${follower.id}/feed/${newTweetRef.id}`)
            .set({ timestamp: parsedTweet.timestamp })
          )
      )
    );
};
